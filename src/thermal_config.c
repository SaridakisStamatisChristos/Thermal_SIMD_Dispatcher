...
static int json_skip(const jsmntok_t *tokens, int token_count, int index) {
    ...
    if (type == JSMN_PRIMITIVE || type == JSMN_STRING || type == JSMN_UNDEFINED) {
        return index + 1;
    }
    int cursor = index + 1;
    while (cursor < token_count && tokens[cursor].parent >= index) {
        cursor = json_skip(tokens, token_count, cursor);
    }
    return cursor;
}

static int json_token_to_string(const char *json, const jsmntok_t *tok, char *dest, size_t dest_size) {
    if (!json || !tok || !dest || dest_size == 0) {
        return -1;
    }
    if (tok->type != JSMN_STRING) {
        return -1;
...
static int json_token_to_bool(const char *json, const jsmntok_t *tok, int *out) {
    ...
    if (len == 4 && strncmp(ptr, "true", 4) == 0) {
        *out = 1;
        return 0;
    }
    if (len == 5 && strncmp(ptr, "false", 5) == 0) {
        *out = 0;
        return 0;
    }
    if (len == 1 && (ptr[0] == '0' || ptr[0] == '1')) {
        *out = (ptr[0] == '1');
        return 0;
    }
    return -1;
...
static int parse_metrics_tls_object(const char *json, const jsmntok_t *tokens, int token_count, int index,
    ...
    if (obj->type != JSMN_OBJECT) {
        return json_skip(tokens, token_count, index);
    }
    int cursor = index + 1;
...
static int parse_metrics_statsd_object(const char *json, const jsmntok_t *tokens, int token_count, int index,
    ...
    while (cursor < end) {
        ...
        if (json_token_equals(json, key, "host")) {
            if (json_token_to_string(json, &tokens[value_index], cfg->statsd_host,
                                     sizeof(cfg->statsd_host)) != 0) {
                return -1;
            }
        } else if (json_token_equals(json, key, "port")) {
            if (json_token_to_int(json, &tokens[value_index], 1, 65535, &cfg->statsd_port) != 0) {
                return -1;
            }
        }
        cursor = next;
...
static int parse_metrics_object(const char *json, const jsmntok_t *tokens, int token_count, int index,
    ...
    while (cursor < end) {
        ...
        if (json_token_equals(json, key, "bind_address")) {
            if (json_token_to_string(json, &tokens[value_index], cfg->metrics_bind_host,
                                     sizeof(cfg->metrics_bind_host)) != 0) {
                return -1;
            }
        } else if (json_token_equals(json, key, "port")) {
            if (json_token_to_int(json, &tokens[value_index], 0, 65535, &cfg->metrics_port) != 0) {
                return -1;
            }
    ...
    while (cursor < end) {
        ...
        if (json_token_equals(json, key, "coeff_path")) {
            if (json_token_to_string(json, &tokens[value_index], cfg->predictive_coeff_path,
                                     sizeof(cfg->predictive_coeff_path)) != 0) {
                return -1;
            }
            ...
            if (json_token_to_int(json, &tokens[value_index], TSD_MIN_TEMP_C, TSD_MAX_TEMP_C,
                                  &cfg->predictive_temp_ceiling_c) != 0) {
                return -1;
            }
            ...
            double alpha = 0.0;
            if (json_token_to_double(json, &tokens[value_index], 0.0, 1.0, &alpha) != 0) {
                return -1;
            }
...
static int parse_config_file(tsd_runtime_config *cfg, const char *path) {
    ...
    if (fseek(fp, 0, SEEK_END) != 0) {
        tsd_log_error(LOG_COMPONENT, "Failed to seek config file '%s'", path);
        fclose(fp);
        return -1;
    }
    long length = ftell(fp);
    if (length < 0) {
        tsd_log_error(LOG_COMPONENT, "Failed to read config file length for '%s'", path);
        fclose(fp);
        return -1;
    }
    if (fseek(fp, 0, SEEK_SET) != 0) {
        tsd_log_error(LOG_COMPONENT, "Failed to rewind config file '%s'", path);
        fclose(fp);
        return -1;
    }
    char *buffer = (char *)malloc((size_t)length + 1);
    if (!buffer) {
        tsd_log_error(LOG_COMPONENT, "Out of memory while reading '%s'", path);
        fclose(fp);
        return -1;
    }
    ...
    if (read != (size_t)length) {
        free(buffer);
        tsd_log_error(LOG_COMPONENT, "Failed to read config file '%s'", path);
        return -1;
    }
    buffer[length] = '\0';
    ...
    while (1) {
        ...
        if (!tokens) {
            free(buffer);
            tsd_log_error(LOG_COMPONENT, "Out of memory while parsing '%s'", path);
            return -1;
        }
        jsmn_init(&parser);
    ...
    if (parsed == 0 || tokens[0].type != JSMN_OBJECT) {
        free(tokens);
        free(buffer);
        tsd_log_error(LOG_COMPONENT, "Top-level JSON in '%s' must be an object", path);
        return -1;
    }
    ...
    for (int pair = 0; pair < tokens[0].size; ++pair) {
        ...
        if (key->type != JSMN_STRING) {
            free(tokens);
            free(buffer);
            tsd_log_error(LOG_COMPONENT, "Invalid key in config file '%s'", path);
            return -1;
        }
        cursor++;
        int value_index = cursor;
...
static int validate_runtime_config(tsd_runtime_config *cfg) {
    ...
    if (cfg->statsd_host[0] == '\0') {
        cfg->statsd_port = 0;
    }
    if (cfg->telemetry_interval_ms < TSD_MIN_TELEMETRY_INTERVAL_MS ||
        cfg->telemetry_interval_ms > TSD_MAX_TELEMETRY_INTERVAL_MS) {
        tsd_log_error(LOG_COMPONENT, "Telemetry interval must be between %d and %d ms",
                      TSD_MIN_TELEMETRY_INTERVAL_MS, TSD_MAX_TELEMETRY_INTERVAL_MS);
        return -1;
    }
    if (cfg->telemetry_max_skew_ms < TSD_MIN_TELEMETRY_SKEW_MS ||
        cfg->telemetry_max_skew_ms > TSD_MAX_TELEMETRY_SKEW_MS) {
        tsd_log_error(LOG_COMPONENT, "Telemetry max skew must be between %d and %d ms",
                      TSD_MIN_TELEMETRY_SKEW_MS, TSD_MAX_TELEMETRY_SKEW_MS);
        return -1;
    }
    if (cfg->predictive_temp_ceiling_c < TSD_MIN_TEMP_C || cfg->predictive_temp_ceiling_c > TSD_MAX_TEMP_C) {
        tsd_log_error(LOG_COMPONENT, "Temp ceiling must be between %d and %d Celsius", TSD_MIN_TEMP_C, TSD_MAX_TEMP_C);
        return -1;
    }
    if (cfg->predictive_safety_margin_c < TSD_MIN_MARGIN_C || cfg->predictive_safety_margin_c > TSD_MAX_MARGIN_C) {
        tsd_log_error(LOG_COMPONENT, "Safety margin must be between %d and %d Celsius", TSD_MIN_MARGIN_C, TSD_MAX_MARGIN_C);
        return -1;
    }
    if (cfg->predictive_emergency_margin_c < TSD_MIN_MARGIN_C ||
        cfg->predictive_emergency_margin_c > TSD_MAX_MARGIN_C) {
        tsd_log_error(LOG_COMPONENT,
                      "Emergency margin must be between %d and %d Celsius",
                      TSD_MIN_MARGIN_C,
                      TSD_MAX_MARGIN_C);
        return -1;
    }
    if (cfg->predictive_alpha < 0.0 || cfg->predictive_alpha > 1.0) {
        tsd_log_error(LOG_COMPONENT, "Predictive alpha must be between 0.0 and 1.0");
        return -1;
    }
    return 0;
}
...
int tsd_runtime_config_parse_cli(tsd_runtime_config *cfg, int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (!strncmp(argv[i], "--config=", 9)) {
            ...
            if (path[0] == '\0') {
                die_invalid_option("--config", path);
            }
            if (parse_config_file(cfg, path) != 0) {
                die_config_error(path, "failed to load configuration");
            }
            ...
            if (value[0] == '\0' ||
                copy_string_field(cfg->metrics_bind_host, sizeof(cfg->metrics_bind_host), value) != 0) {
                die_invalid_option("--metrics-bind", value);
            }
        } else if (!strncmp(argv[i], "--metrics-cert=", 15)) {
            const char *value = argv[i] + 15;
            if (value[0] == '\0' ||
                copy_string_field(cfg->metrics_tls_cert_path, sizeof(cfg->metrics_tls_cert_path), value) != 0) {
                die_invalid_option("--metrics-cert", value);
            }
        } else if (!strncmp(argv[i], "--metrics-key=", 14)) {
            const char *value = argv[i] + 14;
            if (value[0] == '\0' ||
                copy_string_field(cfg->metrics_tls_key_path, sizeof(cfg->metrics_tls_key_path), value) != 0) {
                die_invalid_option("--metrics-key", value);
            }
        } else if (!strncmp(argv[i], "--metrics-ca=", 13)) {
            const char *value = argv[i] + 13;
            if (value[0] == '\0' ||
                copy_string_field(cfg->metrics_tls_ca_path, sizeof(cfg->metrics_tls_ca_path), value) != 0) {
                die_invalid_option("--metrics-ca", value);
            }
            ...
            if (!sep || sep == value || sep[1] == '\0') {
                die_invalid_option("--metrics-basic-auth", value);
            }
            size_t user_len = (size_t)(sep - value);
            size_t pass_len = strlen(sep + 1);
            if (user_len >= sizeof(cfg->metrics_basic_auth_user) ||
                pass_len >= sizeof(cfg->metrics_basic_auth_pass)) {
                die_invalid_option("--metrics-basic-auth", value);
            }
            ...
            const char *value = argv[i] + 14;
            if (value[0] == '\0' ||
                copy_string_field(cfg->statsd_host, sizeof(cfg->statsd_host), value) != 0) {
                die_invalid_option("--statsd-host", value);
            }
        } else if (!strncmp(argv[i], "--statsd-port=", 14)) {
            if (tsd_parse_int_option(argv[i] + 14, 1, 65535, &cfg->statsd_port) != 0) {
                die_invalid_option("--statsd-port", argv[i] + 14);
            }
            ...
            if (tsd_parse_int_option(argv[i] + 15, TSD_MIN_TEMP_C, TSD_MAX_TEMP_C,
                                     &cfg->predictive_temp_ceiling_c) != 0) {
                die_invalid_option("--temp-ceiling", argv[i] + 15);
            }
        } else if (!strncmp(argv[i], "--safety-margin=", 16)) {
            if (tsd_parse_int_option(argv[i] + 16, TSD_MIN_MARGIN_C, TSD_MAX_MARGIN_C,
                                     &cfg->predictive_safety_margin_c) != 0) {
                die_invalid_option("--safety-margin", argv[i] + 16);
            }
        } else if (!strncmp(argv[i], "--emergency-margin=", 19)) {
            if (tsd_parse_int_option(argv[i] + 19, TSD_MIN_MARGIN_C, TSD_MAX_MARGIN_C,
                                     &cfg->predictive_emergency_margin_c) != 0) {
                die_invalid_option("--emergency-margin", argv[i] + 19);
            }
        } else if (!strncmp(argv[i], "--predictive-alpha=", 19)) {
            double alpha = 0.0;
            if (tsd_parse_double_option(argv[i] + 19, 0.0, 1.0, &alpha) != 0) {
                die_invalid_option("--predictive-alpha", argv[i] + 19);
            }
            cfg->predictive_alpha = alpha;
        } else if (!strncmp(argv[i], "--telemetry-interval=", 21)) {
            if (tsd_parse_int_option(argv[i] + 21, TSD_MIN_TELEMETRY_INTERVAL_MS, TSD_MAX_TELEMETRY_INTERVAL_MS,
                                     &cfg->telemetry_interval_ms) != 0) {
                die_invalid_option("--telemetry-interval", argv[i] + 21);
            }
        } else if (!strncmp(argv[i], "--telemetry-max-skew=", 21)) {
            if (tsd_parse_int_option(argv[i] + 21, TSD_MIN_TELEMETRY_SKEW_MS, TSD_MAX_TELEMETRY_SKEW_MS,
                                     &cfg->telemetry_max_skew_ms) != 0) {
                die_invalid_option("--telemetry-max-skew", argv[i] + 21);
            }
        } else if (!strncmp(argv[i], "--telemetry-ewma=", 17)) {
            double ewma = 0.0;
            if (tsd_parse_double_option(argv[i] + 17, 0.0, 1.0, &ewma) != 0) {
                die_invalid_option("--telemetry-ewma", argv[i] + 17);
            }
            cfg->telemetry_ewma_alpha = ewma;
        } else if (!strncmp(argv[i], "--telemetry-profile=", 20)) {
            const char *value = argv[i] + 20;
            if (value[0] == '\0' ||
                copy_string_field(cfg->telemetry_profile_path, sizeof(cfg->telemetry_profile_path), value) != 0) {
                die_invalid_option("--telemetry-profile", value);
            }
        } else if (!strncmp(argv[i], "--coeff-path=", 13)) {
            const char *value = argv[i] + 13;
            if (value[0] == '\0' ||
                copy_string_field(cfg->predictive_coeff_path, sizeof(cfg->predictive_coeff_path), value) != 0) {
                die_invalid_option("--coeff-path", value);
            }
    ...
    if (validate_runtime_config(cfg) != 0) {
        exit(1);
    }
